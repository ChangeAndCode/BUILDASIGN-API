...
const toPlainMasterSyncObject = (value) => {
  if (!value) return {};
...
const isBlankMasterSyncValue = (value) =>
  value === undefined ||
  value === null ||
  String(value).trim() === "";

const getManagedMasterSyncFields = (masterType) => {
  const fields = new Set(
    getCanonicalMasterHeaders(masterType)
      .map((header) => header.mappedField)
      .filter((field) => field && field !== "partNumber"),
  );

  if (fields.has("unitNetWeight")) {
    fields.add("unitNetWeightSourceUnit");
  }

  return fields;
};

const buildStoredMasterSyncRawCells = ({
  masterType,
  incomingRecord,
  currentRecord = null,
  masterHeaders = [],
}) => {
  const managedFields = getManagedMasterSyncFields(masterType);
  const currentRawCells = Array.isArray(currentRecord?.rawCells)
    ? currentRecord.rawCells.map(toPlainMasterSyncObject)
    : [];
  const currentByColumn = new Map(
    currentRawCells.map((cell) => [Number(cell.columnIndex), cell]),
  );
  const knownColumns = new Set();
  const result = [];

  (masterHeaders || []).forEach((storedHeader) => {
    const header = toPlainMasterSyncObject(storedHeader);
    const columnIndex = Number(header.columnIndex);
    if (!Number.isInteger(columnIndex) || columnIndex < 1) return;

    knownColumns.add(columnIndex);
    const currentCell = currentByColumn.get(columnIndex);
    const rule = getMasterHeaderRule(masterType, header.originalName);
    const mappedField = rule?.target || header.mappedField || "";
    const isManaged = Boolean(
      mappedField &&
        (mappedField === "partNumber" || managedFields.has(mappedField)),
    );

    if (!isManaged) {
      if (currentCell) result.push(currentCell);
      return;
    }

    const valueHeader = rule
      ? header
      : getCanonicalMasterHeaders(masterType).find(
          (canonicalHeader) =>
            canonicalHeader.mappedField === mappedField,
        );
    if (!valueHeader) {
      if (currentCell) result.push(currentCell);
      return;
    ...
    const value = getCanonicalCellValue(
      masterType,
      incomingRecord,
      valueHeader,
    );
    if (isBlankMasterSyncValue(value)) return;
  ...
  currentRawCells.forEach((cell) => {
    if (!knownColumns.has(Number(cell.columnIndex))) {
      result.push(cell);
    }
  });
...
const buildStoredMasterSyncRecord = ({
  masterType,
  incomingRecord,
  currentRecord = null,
  masterHeaders = [],
}) => {
  const incoming = toPlainMasterSyncObject(incomingRecord);
  const currentNormalizedValues = toPlainMasterSyncObject(
    currentRecord?.normalizedValues,
  );
  const managedFields = getManagedMasterSyncFields(masterType);

  managedFields.forEach((field) => {
    delete currentNormalizedValues[field];
  });
  ...
  const currentWarnings = Array.isArray(
    currentRecord?.validationWarnings,
  )
    ? currentRecord.validationWarnings
        .map(toPlainMasterSyncObject)
        .filter((warning) => {
          const rule = getMasterHeaderRule(
            masterType,
            warning.field,
          );
          return !rule || !managedFields.has(rule.target);
        })
    : [];
  const incomingWarnings = Array.isArray(incoming.validationWarnings)
    ? incoming.validationWarnings.map(toPlainMasterSyncObject)
    : [];

  return {
    ...incoming,
    normalizedValues,
    rawCells: buildStoredMasterSyncRawCells({
      masterType,
            ...
            (entry) => {
              const recordData = {
                ...buildStoredMasterSyncRecord({
                  masterType,
                  incomingRecord: entry.record,
                  currentRecord: entry.current,
                  masterHeaders: masterFile.headers,
                }),
                sourceRow:
                  entry.current
                    .sourceRow,
 ...
 * entrega lotes pequenos y cada lote se libera despues de insertarse.
 */
  ...
  try {
    ...
    if (!importedMasterFile) {
      throw createMasterServiceError(
        ...
        "No fue posible iniciar la importacion del archivo madre.",
        500,
      );
    }
    ...
    if (importedMasterFile?._id) {
      ...
      await masterFileRepository.updateMasterFileById(
        importedMasterFile._id,
        {
          status: "failed",
          recordCount: 0,
          errorMessage: String(error.message || "Error de importacion").slice(0, 1000),
          updatedBy: adminUserId,
        },
      ).catch(() => {});
    }

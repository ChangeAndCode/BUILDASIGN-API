...
const updateUserAccess = async (req, res) => {
  ...
  if (role !== 'admin' && !VALID_SITES.includes(normalizedSite)) {
  ...
  try {
    ...
    if (
      previousUser &&
      String(previousUser.site || "") !== String(updatedUser.site || "")
    ) {
...
const deleteUser = async (req, res) => {
  ...
  try {
    ...
    const sessionCollection = mongoose.connection.collection('sessions_buildasign');
    let cursor;

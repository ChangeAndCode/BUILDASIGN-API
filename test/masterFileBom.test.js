...
const createBuildasignAliasBomWorkbook = async () => {
...
const assertBuildasignResult = (metadata, records) => {
...
test(
  "B.O.M. BUILDASIGN detecta aliases, fila 3 y columnas C-G sin depender de la hoja",
  async () => {
    const buffer = Buffer.from(
      await createBuildasignAliasBomWorkbook(),
    );
...
test(
  "B.O.M. BUILDASIGN conserva la deteccion al importar por streaming",
  async (t) => {
    const temporaryDirectory = await fs.mkdtemp(
      path.join(os.tmpdir(), "buildasign-bom-alias-"),
    );
    const filePath = path.join(
      temporaryDirectory,
      "bom-buildasign-alias.xlsx",
    );
    const records = [];
    let metadata = null;
    ...
    await fs.writeFile(
      filePath,
      Buffer.from(await createBuildasignAliasBomWorkbook()),
    );
    ...
    assertBuildasignResult(metadata, records);
  },
);

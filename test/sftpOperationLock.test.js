...
test("calcula el vencimiento del lease", () => {
  assert.equal(
...
test("considera activo un envio con lease vigente", () => {
  assert.equal(
    isSftpOperationActive(
      {
        status: "sending",
...
test("considera recuperable un envio cuyo lease vencio", () => {
  assert.equal(
    isSftpOperationActive(
      {
        status: "sending",
...
test("mantiene compatibilidad con locks antiguos", () => {
  assert.equal(
    isSftpOperationActive(
      {
        status: "pending",
...
test("el filtro disponible contempla leases vencidos y locks antiguos", () => {
...
test("el filtro propietario exige el operationId actual", () => {
  assert.deepEqual(
    createSftpOwnerFilter({
      id: "document-id",
...
test("el filtro de envio impide adquirir documentos enviados", () => {
...
test("no elimina el remoto al sobrescribir la misma ruta", () => {
  assert.equal(
    shouldDeletePreviousRemoteFile({
      previousSite: "local-01",
      previousPath: "/out/PL171727_0826.csv",
      nextSite: "local-01",
      nextPath: "/out/PL171727_0826.csv",
    }),
    false,
  );
});

test("elimina el remoto cuando cambia nomenclatura o sede", () => {
  assert.equal(
    shouldDeletePreviousRemoteFile({
      previousSite: "local-01",
      previousPath: "/out/PL171727_0826.csv",
      nextSite: "local-01",
      nextPath: "/out/PL181030_0826.csv",
    }),
    true,
  );
  assert.equal(
...
test("no elimina un remoto anterior inexistente", () => {
  assert.equal(
    shouldDeletePreviousRemoteFile({
      previousSite: "local-01",
      previousPath: "",
